...
(function () {
  ...
  const DICT = {
    ...
    btn_batch: ['Grab in batch · up to 4 at once', '批次擷取 · 最多同時 4 個'],
    btn_onebyone: ['Grab one by one', '逐一擷取'],
    hint_batch: [
      'batch opens the selected feeds side by side and grabs them together (first 4 selected) · one by one visits each feed page in turn, any number — either way, keep the tab visible',
      '批次：將所選動態並排開啟並同時擷取（取所選前 4 個）· 逐一：依序造訪每個動態頁面，數量不限 — 兩種方式都需保持分頁可見',
    ],
    ...
    opt_reply_any: ['💬 any', '💬 不限'],
    title_min_replies: ['minimum reply count', '最少回覆數'],
    title_date_range: ['posted between (takenAt)', '發佈日期範圍'],
    title_clear_dates: ['clear date range', '清除日期範圍'],
    opt_sort_capture: ['Capture order', '擷取順序'],
    ...
    shown_of: ['of {n}', '（共 {n} 篇）'],
    shown_hidden_replies: [
      '· {n} hidden (no reply data — captured before v0.8.6, re-grab)',
      '· 隱藏 {n} 篇（無回覆資料 — v0.8.6 前擷取，請重新擷取）',
    ],
