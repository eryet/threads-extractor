...
| Post engagers (who liked / reposted) | `BarcelonaFeedbackHubTabQuery` (doc_id rotates), connection `data.feedback_hub_tab_items`; variables `{post_id: <numeric pk>, tab_type: 'like' or 'repost', sort_type: 'default'}` + relay providers; node = `{actor:{username, pk}, extra:{icon_name, context}, timestamp}`, standard `page_info`. The listed actors are a **partial** subset of the count (only accounts onboarded to Threads). Fetched on demand from the dashboard — see below |
